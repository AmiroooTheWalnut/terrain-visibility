function [selectedGaurds] = problem3_fcn(isParallel,sizeTerrain,numCandidateGaurds,guardHeight,figureIndex)
...
for i=2:size(selectedGaurds,2)
    orig=[candidateGaurds(1,selectedGaurds(i)),1,guardHeight];
    [visibleFaces,visibleVerticesSizes] = calcVisibility(orig,vertices,faces,X,Y,terrainPoints,isParallel);
    generatedX=[];
    generatedY=[];
    generatedZ=[];
    ...
    scatter3(candidateGaurds(1,selectedGaurds(i)),1,guardHeight,'MarkerEdgeColor',colors(i,:),'MarkerFaceColor',colors(i,:))
end
