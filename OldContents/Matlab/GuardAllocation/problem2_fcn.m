function [selectedGaurds,pi_v,d] = problem2_fcn(isParallel,sizeTerrain,numCandidateGaurds,guardHeight,figureIndex)
...
for f=1:3
    for j=1:1
        for i=1:numCandidateGaurds
            for k=i+1:numCandidateGaurds
                if i~=k
                    bw=[];
                    bw(size(terrainPoints,1),size(terrainPoints,2))=0;
                    bw=logical(bw);
                    orig=[candidateGaurds(1,i),1,guardHeight];
                    ...
                    orig=[candidateGaurds(1,k),1,guardHeight];
                    [visibleFaces,visibleVerticesSizes] = calcVisibility(orig,vertices,faces,X,Y,terrainPoints,isParallel);
                    ...
                    for o=1:size(CC.PixelIdxList,2)
                        ...
                        isValid=1;
                        currentDValue=inf;
                        for h=size(pi_v,2):-1:2
                            if d(1,h)<minVal+1 && d(1,h)<currentDValue
                                currentDValue=d(1,h);
                                if i==pi_v{1,h}(1,1) || k==pi_v{1,h}(1,2) || i==pi_v{1,h}(1,2) || k==pi_v{1,h}(1,1)
                                    isValid=0;
                                    break;
                                end
                            end
%                             if d(1,h)>minVal+1
%                                 if d(1,h-1)>0
%                                     if isempty(pi_v{1,h-1})==0
%                                         if d(1,h-1)>minVal+1
%                                             if i==pi_v{1,h-1}(1,1) || k==pi_v{1,h-1}(1,2)
%                                                 isValid=0;
%                                             end
%                                         end
%                                     end
...
selectedGaurds=drawSolution(vertices,faces,X,Y,terrainPoints,d,pi_v,candidateGaurds,guardHeight,isParallel);
end

function selectedGaurds=drawSolution(vertices,faces,X,Y,terrainPoints,d,pi_v,candidateGaurds,guardHeight,isParallel)
...
for i=1:numGaurds
    ...
    orig=[candidateGaurds(1,selectedGaurds{1,i}(1,1)),1,guardHeight];
    [visibleFaces,visibleVerticesSizes] = calcVisibility(orig,vertices,faces,X,Y,terrainPoints,isParallel);
    generatedX=[];
    generatedY=[];
    generatedZ=[];
    ...
    scatter3(candidateGaurds(1,selectedGaurds{1,i}(1,1)),1,guardHeight,'MarkerEdgeColor',colors(i,:),'MarkerFaceColor',colors(i,:))
    
    orig=[candidateGaurds(1,selectedGaurds{1,i}(1,2)),1,guardHeight];
    [visibleFaces,visibleVerticesSizes] = calcVisibility(orig,vertices,faces,X,Y,terrainPoints,isParallel);
    generatedX=[];
    generatedY=[];
    generatedZ=[];
    ...
    scatter3(generatedX,generatedY,generatedZ+i,'MarkerEdgeColor',colors(i,:),'MarkerFaceColor',colors(i,:))
    scatter3(candidateGaurds(1,selectedGaurds{1,i}(1,2)),1,guardHeight+i,'MarkerEdgeColor',colors(i,:),'MarkerFaceColor',colors(i,:))
