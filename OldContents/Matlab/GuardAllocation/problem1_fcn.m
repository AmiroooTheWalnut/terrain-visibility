function [selectedGaurds,pi_v,d] = problem1_fcn(isParallel,sizeTerrain,numCandidateGaurds,guardHeight,figureIndex)
...
for i=1:numGaurds
    if selectedGaurds(i,1)~=0
        orig=[candidateGaurds(1,selectedGaurds(i,1)),1,guardHeight];
        [visibleFaces,visibleVerticesSizes] = calcVisibility(orig,vertices,faces,X,Y,terrainPoints,isParallel);
        generatedX=[];
        generatedY=[];
        generatedZ=[];
        ...
        scatter3(candidateGaurds(1,selectedGaurds(i,1)),1,guardHeight,'MarkerEdgeColor',colors(i,:),'MarkerFaceColor',colors(i,:))
    end
end
