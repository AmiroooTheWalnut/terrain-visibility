...
prg = cl.Program(ctx, """
__kernel void sum(
    int width, int height, int gX, int gY, float gZ, __global float *tMat, __global float *zBuff, __global char *visRes, float range)
{
...
knl(queue, global_size, None, np.int32(widthT), np.int32(heightT), np.int32(gX), np.int32(gY), np.float32(gZ), terrain_b, zBuff_b, visRes_b, np.float32(range))
